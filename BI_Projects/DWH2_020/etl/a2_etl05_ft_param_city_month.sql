...
WITH base AS (
    ...
    JOIN dwh2_020.dim_param dp ON dp.param_key = re.paramid
),
...
thresholds AS (
    SELECT 
        pa.paramid,
        MAX(CASE WHEN pa.alertid = 1001 THEN pa.threshold END) AS thr_yellow,   -- Yellow Threshold
        MAX(CASE WHEN pa.alertid = 1002 THEN pa.threshold END) AS thr_orange,   -- Orange Threshold
        MAX(CASE WHEN pa.alertid = 1003 THEN pa.threshold END) AS thr_red,      -- Red Threshold
        MAX(CASE WHEN pa.alertid = 1004 THEN pa.threshold END) AS thr_crimson   -- Crimson Threshold
...
daily_rank AS (
    ...
    FROM base b
    JOIN thresholds th ON th.paramid = b.param_key   -- Join to get thresholds
...
exceed AS (
    SELECT
        ...
        city_key,
        param_key,
        COUNT(DISTINCT day) AS exceed_days_any
    FROM daily_rank
    WHERE daily_rank >= 1
    GROUP BY month_key, city_key, param_key
)
...
-- -----------------------------------------
-- 7. Final insert with sequence-generated ft_pcm_key
-- -----------------------------------------
